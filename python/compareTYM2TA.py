...
def signal_handler(signum, frame):
    print("Received termination signal. Cleaning up...", flush=True)
    if 'p' in locals():
        p.terminate()
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--only_test_number",default = None, help="only_test_number", type=str)
    ...
    for map_file_path in map_file_paths:
        if (args.only_test_number):
            if not ("_"+args.only_test_number+"_" in map_file_path):
                continue
